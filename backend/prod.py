...
# Updated prompt: also provide definitions of the recommended methods.
...
def generate_concise_query(input_query):
    ...
    return concise_response.strip().replace('"', '')
...
def run_chatbot(query):
    if query:
        ...
        # Get original documents but don't display initial answer
        original_docs = st.session_state.vectors.similarity_search(query, k=5)

        # Generate a concise query for Google Scholar
